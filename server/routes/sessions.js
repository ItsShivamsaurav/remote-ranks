...
router.get('/me', requireAuth, async (req, res) => {
  ...
  const [rows] = await pool.query(
    `SELECT s.session_id, s.user1_id, s.user2_id, s.subject_id, s.scheduled_time, s.duration_minutes, s.meeting_link, s.status,
            u1.name AS user1_name, u2.name AS user2_name, subj.subject_name
     FROM sessions s
...
router.post('/propose', requireAuth, async (req, res) => {
  ...
  // Generate random Jitsi room name
